...
Что привело тебя к размышлениям о любви? Есть что-то особенное в твоей жизни, что заставляет задуматься об этом?`;
  ...
  generateGeneralConversation(lowerInput) {
    // ✅ СПЕЦИАЛЬНАЯ ОБРАБОТКА ВОПРОСА О ШИФРОВАНИИ
    if (lowerInput.includes('шифрование') || lowerInput.includes('шифрования') || lowerInput.includes('криптография')) {
      return `Шифрование - это процесс преобразования информации в код для защиты от несанкционированного доступа! 🔐

**Основные принципы:**
• **Симметричное шифрование** - один ключ для шифрования и расшифровки
• **Асимметричное шифрование** - пара ключей (открытый и закрытый)
• **Хеширование** - односторонние функции для проверки целостности

**Популярные алгоритмы:**
• **AES** - Advanced Encryption Standard (симметричный)
• **RSA** - алгоритм с открытым ключом
• **SHA-256** - криптографическая хеш-функция
• **HTTPS** - защищенный протокол для веб-трафика

**Применение:**
• Защита личных данных в интернете
• Безопасность банковских операций
• Шифрование мессенджеров и email
• Защита файлов на устройствах

Шифрование - основа современной информационной безопасности!`;
    }
